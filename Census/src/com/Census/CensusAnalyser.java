...
import com.Census.CensusAnalyserException.ExceptionType;
...
public class CensusAnalyser {
    ...
    public int loadIndianStateCode(String csvFilePath) throws CensusAnalyserException {
        ...
        try {
        	...
        	Reader reader = Files.newBufferedReader(Paths.get(csvFilePath));
            CsvToBeanBuilder<IndiaStateCSV> csvToBeanBuilder = new CsvToBeanBuilder<IndiaStateCSV>();
            csvToBeanBuilder.withType(IndiaStateCSV.class);
            csvToBeanBuilder.withIgnoreLeadingWhiteSpace(true);
            CsvToBeanBuilder<IndiaCensusCSV> csvToBean = csvToBeanBuilder.build();
            Iterator<IndiaCensusCSV> stateCodesCSVIterator = csvToBean.iterator();
            
            int numberOfEntries = 0;
    		while(stateCodesCSVIterator.hasNext()) {
    			numberOfEntries++;
    			IndiaCensusCSV censusData = stateCodesCSVIterator.next();
    		}
    		return numberOfEntries;
        } 
        catch (IOException e) {
